...
program:
  PROGRAM {root = new_subtree(PROGRAM_NODE,NO_TYPE,20);}ID SEMI vars-sect stmt-sect
;
...
AST* unify_bin_op(Ast* l, Ast* r, const char* op){
	if(r == NULL || l == NULL){
		return NULL;
	}		
	Type type_new_node = unify_bin_op(get_node_type(l),get_node_type(r),op,unify);
