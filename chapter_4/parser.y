...
assign-stmt:
  ID {exists_var();}ASSIGN expr SEMI { $$ = unify_assign($1,$3);}
;
...
expr:
  expr LT expr { $$ = unify_LT($1,$3);}
| expr EQ expr { $$ = unify_EQ($1,$3);}
| expr PLUS expr {$$ = unify_plus($1,$3);}
| expr MINUS expr {$$ = unify_minus($1,$3);}
| expr TIMES expr {$$ = unify_times($1,$3);}
| expr OVER expr {$$ = unify_over($1,$3);}
| LPAR expr RPAR
| TRUE
| FALSE
| INT_VAL
